(function() {
    $("a.upload").live('click', function() {
        createForm("/radiomanager/ajaxUpload", {}, 500, 220);
        return false;
    });
})();
    ...
    $(".rm_input_files").livequery(function()
    {
        $(this).on('change', function(event)
        {
            $.each(event.target.files, function(i, file)
            {
                ...
                {
                    showPopupInfo("One or more files has unsupported file size. Maximum is 512 MB!");
                }
            });
            if (procFlag === false)
            {
        ...
        uploadHandle = $.ajax({
            ...
            success: function(data) {
                ...
                try
                {
                    ...
                    {
                        showPopupInfo(file.name + ": " + json.code, 1);
                    }
                }
                catch (e)
                {
